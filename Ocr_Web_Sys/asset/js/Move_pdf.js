...
function preview(e) {
    ...
    if (Select_Bank.value != "") {
        ...
        //   }
        var aa = Paturn_Match(0, Select_Bank.textContent);
        console.log(aa);
...
$(function() {
    // 操作盤のドラッグ、リサイズ
    $("#Sosa_IMg,.img_select,.bunkatu_img_box").draggable();
    $("#Sosa_IMg,.bunkatu_img_box").resizable({
        minHeight: $(".ims_sosa_box").height() / 4,
        minWidth: $(".ims_sosa_box").width() / 4,
        maxHeight: $(".ims_sosa_box").height() * 2,
        maxWidth: $(".ims_sosa_box").width() * 2,
        minHeight: $(".ims_sosa_box").height() / 2,
        minWidth: $(".ims_sosa_box").width() / 2,
        maxHeight: $(".ims_sosa_box").height() / 1.1,
        maxWidth: $(".ims_sosa_box").width(),
    });
...
$(function() {
    ...
    $('#Select_Bank').change(function() {
        ...
        tr.id = "Mask_table_tr0";
        table.appendChild(tr);
        //   col += 1;
        for (var i = 0; i < col; i++) {
            var td = document.createElement('td');
            td.id = "Mask_table_td_row0_col" + i;
            tr.appendChild(td);
        }
...
function Chosei_Conplete() {
    //  現在設定記号
    var kigo_id = document.getElementById("Now_kigo");
    //  //  現在記号が空でない処理
    //  if (kigo_id.value != "") {
    // 座標足していく変数 
    var x_i = 0;
    ...
    //  //  クリッピングマスクの高さ指定
    //  var Mask_Table_height = tr_total_size + tr_total_size + tr_id.clientWidth;
    ...
    canvas.height = Mask_Table.clientWidth + Mask_Table.clientHeight + border_size * (row + 2);
    //  canvas.height = Mask_Table.clientHeight;
    ...
    for (var l = 0; l < row; l++) {
        ...
        for (var i = 0; i < col; i++) {
            ...
            // クリップした画像の先頭に@付与
            context.fillText(kigo_id.value, 0, img_y + border_size, y_zahyo);
            // クリップした画像の先頭に@付与
            // context.fillText(" ", 17, img_y, y_zahyo);
        }
    ...
    //  画像ポストする
    //  img_Dl(canvas, img_id.id);

    // } else {
    //     alert("現在記号が空になります。記号を入力してください。")
    // }
}
